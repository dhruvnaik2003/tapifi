...
export default function NfcVerifyScreen({ navigation }: any) {
  ...
  const scanChip = async () => {
    if (hasNfc === false) {
      ...
      setTimeout(() => {
        setIsScanning(false);
        verifyChip('mock-uid-emulator-123');
      }, 2000);
      return;
    }
  ...
  const verifyChip = async (uid: string) => {
    setLoading(true);
    try {
      const response = await apiClient.post('/api/nfc/verify', { uid });
...
const styles = StyleSheet.create({
  ...
  simulateButton: { marginTop: theme.spacing.md, paddingVertical: 12, backgroundColor: '#333', borderRadius: 20, paddingHorizontal: 20 },
